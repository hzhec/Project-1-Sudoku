...
const clearBox = document.querySelector(".clear-selector");
...
const hintBox = document.querySelector(".hint-selector");
	...
	for (let i = 0; i < 81; i++) {
		...
		if (num !== 0) {
			...
			squares[i].addEventListener("click", (event) => {
				// if (checkDuplicates(array, i, selectedSquare)) {
				// }
				if (hintStatus) {
					...
					numOfHint--;
					hintBox.innerText = `Hint: ${numOfHint}`;
					if (numOfHint === 0) {
						hintBox.disabled = true;
					}
					activeBtn.classList.remove("selected");
					hintStatus = false;
				}
